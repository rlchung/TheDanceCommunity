    ...
    request("https://graph.facebook.com/" + page_id + "?fields=name,id,cover,emails,link,bio,description,about,personal_info,general_info,awards&access_token=" + token, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            request("https://graph.facebook.com/" + page_id + "/photos?fields=images&access_token="+ token, function (error, response, body){
                if (!error && response.statusCode == 200) {
                    var profile_pic_json = JSON.parse(body);
                    request("https://graph.facebook.com/" + info_json["cover"]["id"] + "?fields=webp_images&access_token=" + token, function(error, response, body){
                        if (!error && response.statusCode == 200) {
                            var cover_pic_json = JSON.parse(body);
                            
                            var name                = info_json["name"],
                                id                  = info_json["id"],
                                cover_pic           = cover_pic_json["webp_images"][0]["source"],
                                ...
                                profile_pic         = profile_pic_json["data"][0]["images"][0]["source"];
...
function delete_team(page_id){
    Team.findOneAndRemove({fb_id:page_id},function(error, teamObj){
        if(error){
            console.log(error);
        // if teamObj exists 
        } if(teamObj){
            ...
            console.log(page_id + " DOES NOT EXIST");
        }
    });
};
